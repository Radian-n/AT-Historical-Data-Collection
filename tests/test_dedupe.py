...
class TestDedupe:
    ...
    def test_no_duplicates(self) -> None:
        ...
        key_cols = [Columns.VEHICLE_ID, Columns.FEED_TIMESTAMP]
    ...
    def test_same_key_different_data(self) -> None:
        ...
        assert len(result) == 1
        assert result[0][Columns.LABEL] == "First Label"
    ...
    def test_single_row(self) -> None:
        ...
        assert len(result) == 1
        assert result[0][Columns.VEHICLE_ID] == "v1"
    ...
    def test_none_in_key_raises_error(self) -> None:
        ...
        rows = [
            make_row("v1", ts),
            {
                Columns.VEHICLE_ID: None,
                Columns.FEED_TIMESTAMP: ts,
                Columns.LABEL: "Bus",
            },
        ]
        key_cols = [Columns.VEHICLE_ID, Columns.FEED_TIMESTAMP]
    ...
    def test_preserves_order(self) -> None:
        ...
        assert len(result) == 3
        assert result[0][Columns.VEHICLE_ID] == "v3"
        assert result[1][Columns.VEHICLE_ID] == "v1"
        assert result[2][Columns.VEHICLE_ID] == "v2"
